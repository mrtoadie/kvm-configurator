...
/* -------------------------------------------------
   Datenstruktur für die VM‑Konfiguration
   ------------------------------------------------- */
...
/* -------------------------------------------------
   Mapping von Distro‑Namen → virt‑install‑Variant
   ------------------------------------------------- */
...
func buildVirtInstallCmd(name, ram, cpus, diskPath, iso, variant string, diskSizeGB int) []string {
	return []string{
		...
		"--cdrom", iso,
		"--boot", "hd",
		//"--print-xml", // wir wollen nur das XML zurück
...
/* -------------------------------------------------
   Render‑Hilfsfunktion (Tab‑Writer)
   ------------------------------------------------- */
...
iso = "/run/media/toadie/data/ISOs/ubuntu-24.04.3-desktop-amd64.iso"
	...
	// ---------- 4️⃣ VM bei libvirt registrieren ----------
...
/* -------------------------------------------------
   main – Prompt + VM‑Erstellung
   ------------------------------------------------- */
func main() {
	// Ausgangskonfiguration – kann vom Nutzer im Prompt verändert werden
	cfg := DomainConfig{
		Name:    "my‑guest",
		MemMiB:  1024,
		VCPU:    2,
	...
	// 1️⃣ Interaktives Formular, um Werte anzupassen
	promptForm(&cfg)

	// 2️⃣ Abschließende Übersicht (Tab‑Writer)
	...
	// 3️⃣ VM anlegen (virsh/virt‑install)
