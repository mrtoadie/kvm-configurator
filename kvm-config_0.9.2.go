...
func chooseDistro(r *bufio.Reader) (distro, error) {
	...
	if line != "" {
		if i, e := strconv.Atoi(line); e == nil && i >= 1 && i <= len(sorted) {
			...
			return distro{}, fmt.Errorf("\x1b[31mInvalid selection\x1b[0m")
		}
	}
...
func runNewVMWorkflow() {
	...
	if err != nil {
		log.Fatalf("\x1b[31mOS selection failed: %v\x1b[0m", err)
	}
	...
	if err = createVM(cfg, variant); err != nil {
		fmt.Fprintln(os.Stderr, "Error generating the VM:", err)
		os.Exit(1)
	}
}
